...
Emoji mode uses `🧠` when Hindsight is connected and `🤯` after failed recall/retain/bank access. `status.detail: "minimal"` hides all text after the icon. `status.detail: "project"` shows the bank ID, `"activity"` shows recall/retain activity without the bank ID, and `"verbose"` shows project, bank ID, and activity. `status.maxLength` caps displayed text, and `status.showActivity` controls whether recall/retain activity replaces the idle text.
